...
NetworkConnectionState GSMConnectionHandler::update_handleInit()
{
  ...
  while (!connected) {
    mkr_gsm_feed_watchdog();
    Debug.print(DBG_INFO, F("LOOP"));
    Debug.print(DBG_INFO, _apn);
    if ((_gsm.begin(_pin) == GSM_READY) && (_gprs.attachGPRS("super", _login, _pass) == GPRS_READY)) {
      Debug.print(DBG_INFO, F("CONNECTED"));
      connected = true;
    } else {
      Debug.print(DBG_INFO, F("NOT CONNECTED"));
      delay(1000);
    }
  }
  return NetworkConnectionState::CONNECTING;
