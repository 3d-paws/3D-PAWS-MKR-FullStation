...
void SD_ReadConfigFile() {
  ...
  cf_instrument_id  = SD_findCharStr(F("instrument_id"));
  sprintf(msgbuf, "CF:%s=[%s]", F("instrument_id"), cf_instrument_id); Output (msgbuf);
  ...
  cf_aes_myiv    = SD_findCharStr(F("aes_myiv"));
  sprintf(msgbuf, "CF:%s=[%s]", F("aes_myiv"), cf_aes_myiv); Output (msgbuf);
}
